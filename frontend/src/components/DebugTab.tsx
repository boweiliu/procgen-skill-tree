...
export function DebugTab({
  ...
  const canBeAllocated: string = canAllocate(
    ...
    playerSave.activeQuest !== undefined
  );
  ...
  return (
    <>
      ...
      <div>
        <button
          onClick={() => {
            ...
            setB64PlayerSaveString(
              btoa(JSON.stringify(playerSave, undefined, 2))
            );
          }}
        >
          export to base64{' '}
      ...
      <h3>Current Node</h3>
      <div>Z={selectedPointNode.z}</div>
      <div>
        Chunk={selectedPointNode.chunkCoord.x},{selectedPointNode.chunkCoord.y}
      </div>
      <div>
        Node={selectedPointNode.pointNodeCoord.x},
        {selectedPointNode.pointNodeCoord.y}
      </div>
      <br></br>
      <div>
        Allocated? <br />
      ...
      {playerSave.questProgressHistory.length === 0 ? (
        <> empty </>
      ) : (
          ...
          return (
            <div key={i}>
              <div>
                ...
                Node={pointNodeRef.pointNodeCoord.x},
